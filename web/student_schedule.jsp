...
<body class="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-100">
  <div class="max-w-6xl mx-auto p-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div class="lg:col-span-1 rounded-xl border border-slate-800 bg-slate-900/60 p-4 h-fit">
        <div class="flex items-center justify-between mb-3">
          <h2 class="font-semibold">Announcements</h2>
          <button id="clearAnnouncementsBtn" class="text-xs px-2 py-1 rounded bg-slate-800 hover:bg-slate-700">Clear</button>
        </div>
      ...
      <div class="lg:col-span-2">
        ...
        <div class="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
          <div class="flex items-center justify-between gap-3 mb-4">
            <button id="prevDay" class="rounded-lg bg-slate-800 hover:bg-slate-700 px-3 py-2 text-sm">Previous</button>
            <div id="currentDayLabel" class="text-lg font-semibold"></div>
            <div class="flex items-center gap-2">
              <button id="todayBtn" class="rounded-lg bg-cyan-500 hover:bg-cyan-400 text-slate-900 px-3 py-2 text-sm font-semibold">Today</button>
              <button id="nextDay" class="rounded-lg bg-slate-800 hover:bg-slate-700 px-3 py-2 text-sm">Next</button>
            </div>
          </div>

          <div id="dayStrip" class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2 mb-4"></div>
...
<script>
  ...
  let grouped = {};
  let dayTypeMap = {};
  let clearedAnnouncementUntil = Number(localStorage.getItem('eduflow_announcements_cleared_until') || '0');
  ...
  function renderDayStrip() {
    ...
    uiDays.forEach((day, idx) => {
      ...
      btn.className = 'rounded-lg px-3 py-2 text-sm border text-left transition ' +
        (active ? 'bg-cyan-500/20 text-cyan-100 border-cyan-400' : 'bg-slate-900/70 text-slate-200 border-slate-700 hover:bg-slate-800');
      let meta = '';
      if (type === 'WEEKEND') meta = '<div class="text-xs text-amber-300 mt-0.5">Weekend</div>';
      if (type === 'HOLIDAY') meta = '<div class="text-xs text-rose-300 mt-0.5">Holiday</div>';
      if (type === 'WORKING' && isToday) meta = '<div class="text-xs text-cyan-300 mt-0.5">Today</div>';
  ...
  function renderDaySchedule() {
    ...
    if (type === 'WEEKEND' || type === 'HOLIDAY') {
      const cls = type === 'WEEKEND' ? 'border-amber-500/40 bg-amber-500/10 text-amber-200' : 'border-rose-500/40 bg-rose-500/10 text-rose-200';
      dayNotice.innerHTML = '<div class=\"rounded-lg border p-3 text-sm ' + cls + '\">' + day + ' is marked as ' + type + ' by admin.</div>';
    } else {
      dayNotice.innerHTML = '';
    }
    ...
    if (items.length === 0) {
      daySchedule.innerHTML = '<div class="rounded-lg border border-slate-800 bg-slate-950/40 p-4 text-slate-400">No classes scheduled for ' + day + '.</div>';
      return;
    }
    items.forEach(item => {
      const card = document.createElement('div');
      card.className = 'rounded-lg border border-slate-800 bg-slate-950/50 p-4';
      card.innerHTML =
        '<div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2">' +
          '<div>' +
            '<div class="text-base font-semibold">' + item.subject + '</div>' +
            '<div class="text-sm text-slate-400">Teacher: ' + item.teacher + '</div>' +
          '</div>' +
          '<span class="px-2 py-1 rounded-full text-xs ' + statusClass(item.status) + '">' + item.status + '</span>' +
        '</div>' +
        '<div class="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">' +
          '<div class="rounded-md bg-slate-900/70 px-3 py-2">Time: ' + item.timeStart + ' - ' + item.timeEnd + '</div>' +
          '<div class="rounded-md bg-slate-900/70 px-3 py-2">Room: ' + item.room + '</div>' +
        '</div>';
  ...
  async function loadAnnouncements() {
    try {
      ...
      if (visibleCount === 0) {
        announcementList.innerHTML = '<li class=\"text-slate-500\">No announcements to show</li>';
      }
    } catch (e) {
      announcementList.innerHTML = '<li class=\"text-slate-500\">Failed to load announcements</li>';
    }
  }
  ...
  clearAnnouncementsBtn.addEventListener('click', () => {
    ...
    fetch(ctx + '/announcements')
      .then(res => res.json())
      .then(data => {
        const maxId = Math.max(0, ...(data.items || []).map(a => parseInt(a.id || '0', 10)));
        clearedAnnouncementUntil = maxId;
        localStorage.setItem('eduflow_announcements_cleared_until', String(clearedAnnouncementUntil));
        announcementList.innerHTML = '<li class=\"text-slate-500\">No announcements to show</li>';
      })
      .catch(() => {
        announcementList.innerHTML = '<li class=\"text-slate-500\">No announcements to show</li>';
